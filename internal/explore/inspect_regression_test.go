...
func TestInspectEndpointAlwaysUpdatesInspector(t *testing.T) {
	...
	outDir := t.TempDir()
	htmlPath := filepath.Join(outDir, "inspect-regression.html")
	if err := os.WriteFile(htmlPath, []byte(inlineInspectRegressionDocument(t, inspectRegressionPayload())), 0o600); err != nil {
	...
	report := inspectRegressionReport(string(out))
	if !report.ready {
		t.Fatalf("inspect regression script did not finish\n%s", out)
	}
	if report.failures != 0 {
		t.Fatalf("expected Inspect endpoint to update inspector + preserve active subtab, got %d failures\n%s", report.failures, report.detail)
	}
}

func inlineInspectRegressionDocument(t *testing.T, payload *Payload) string {
	t.Helper()
...
<div id="inspect-regression" hidden data-ready="false" data-failures="-1"></div>
<script>
`+string(jsBytes)+`
</script>`+inspectRegressionHarness(),
		1,
	)
	return doc
}

func inspectRegressionHarness() string {
	return `
<script>
(function () {
  function node() { return document.getElementById('inspect-regression'); }
  ...
  function run() {
    ...
    window.setTimeout(function () {
      // Expand /tax-rule endpoints to create an inline inspector mount.
      ...
      window.setTimeout(function () {
        ...
        window.setTimeout(function () {
          ...
          click('button.endpoint-diag-tab[data-endpoint-subtab="summary"]');

          window.setTimeout(function () {
            var beforeTab = activeSubtab();
            if (beforeTab !== 'summary') {
              failures.push({ kind: 'precondition', expected: 'summary', got: beforeTab });
            }
            ...
            window.setTimeout(function () {
              ...
              if (code.indexOf('GET /tax-rule/{id}') === -1) {
                failures.push({ kind: 'inspector-endpoint', expected_includes: 'GET /tax-rule/{id}', got: code });
              }

              var afterTab = activeSubtab();
              if (afterTab !== beforeTab) {
                failures.push({ kind: 'subtab-changed', expected: beforeTab, got: afterTab });
              }

              // Selected state should be obvious after selection.
              if (!document.body.classList.contains('has-endpoint-selection')) {
                failures.push({ kind: 'selection-class-missing' });
              }
...
func inspectRegressionReport(dom string) inspectReport {
	readyRe := regexp.MustCompile(`id="inspect-regression"[^>]*data-ready="([^"]+)"`)
	failRe := regexp.MustCompile(`id="inspect-regression"[^>]*data-failures="([^"]+)"`)
	textRe := regexp.MustCompile(`id="inspect-regression"[^>]*>([^<]*)</div>`)
	...
	return inspectReport{ready: ready, failures: failures, detail: detail}
}

func inspectRegressionPayload() *Payload {
	...
	details := map[string]EndpointDetail{
		...
		// Intentionally omit endpointDetails for tax-rule-detail to cover the "Inspect does nothing"
		// case when endpointDetails is missing for a visible endpoint row.
	}
